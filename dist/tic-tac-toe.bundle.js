...
(function () {
    ...
    angular.module('ticTacToe').service('MessageService', MessageService);
    ...
    function MessageService() {
        ...
        MessageService.lossMessages = ['AW SHUCKS!', 'I GOT DISTRACTED...', 'HOW IN THE?!', 'ARE YOU CHEATING?', 'I BET YOU WON\'T WIN AGAIN.', 'WE ALL GET LUCKY.', 'DOUBLE OR NOTHING.', 'REMATCH?', 'BEGINNER\'S LUCK!'];

        MessageService.tieMessages = ['CAT!', 'WE TIED!', 'UGH THIS AGAIN.', 'YOU PLAY LIKE A COWARD!', 'WE TIED AGAIN!'];

        MessageService.winMessages = ['HAHA LOSER!', 'NEXT!', '*YAWN*', 'THAT WAS CUTE.', 'IS THAT ALL YOU\'VE GOT?'];

        MessageService.getLossMessage = getLossMessage;
        function getLossMessage() {
            return MessageService.selectMessage(MessageService.lossMessages);
        }

        MessageService.getTieMessage = getTieMessage;
        function getTieMessage() {
            return MessageService.selectMessage(MessageService.tieMessages);
        }

        MessageService.getWinMessage = getWinMessage;
        function getWinMessage() {
            return MessageService.selectMessage(MessageService.winMessages);
        }
        ...
        function selectMessage(messages) {
            ...
            return messages[Math.round(random * messages.length)];
        }
    ...
    function NodeService(TicTacToeService) {
        ...
        function Node(cells, value, move, parent) {
            ...
            Node.expand = function () {
                ...
                var grandChildrenValue = TicTacToeService.getOtherValue(this.value);
                ...
                for (var i = 0; i < numberOfAvailableMoves; i++) {
                    ...
                    var newChild = NodeService.getNode(tempCells, grandChildrenValue, move, this);
...
(function () {
    ...
    function TicTacToeController(MessageService, OpponentService, TicTacToeService) {
        ...
        function select(cell) {
            if (TicTacToeController.isGameOver) {
                TicTacToeController.showDialogue(false, 'THE GAME IS ALREADY OVER.');
                ...
                if (TicTacToeController.cells[cell] === -1) {
                    ...
                    TicTacToeController.showDialogue(false, 'YOU CANNOT GO THERE!');
                }
            } else {
                TicTacToeController.showDialogue(false, 'SIMMER DOWN! IT\'S MY TURN STILL!');
            }
        }
