...
gulp.task('embed-scoreboard', function () {
    gulp.src('./www/js/scoreboard/directive/*.js')
        .pipe(embedTemplates())
        .on('error', errorWarning)
        .pipe(gulp.dest('./www/js/scoreboard'));
});
...
gulp.task('embed-ticTacToe', function () {
    gulp.src('./www/js/ticTacToe/directive/*.js')
        .pipe(embedTemplates())
        .on('error', errorWarning)
        .pipe(gulp.dest('./www/js/ticTacToe'));
});
...
gulp.task('scripts', function() {
    return gulp.src([
        ...
        .pipe(concat('tic-tac-toe.full.js'))
        .pipe(gulp.dest('./dist'))
        .pipe(sourcemaps.init())
        .pipe(rename('tic-tac-toe.min.js'))
        .pipe(uglify())
        .on('error', errorWarning)
...
gulp.task('browserify-task', function() {
    return browserify('./dist/index.js')
        ...
        .bundle()
        .on('error', errorWarning)
        .pipe(source('tic-tac-toe.bundle.js'))
        .pipe(buffer())
...
gulp.task('watch', function() {
    ...
    gulp.watch('./dist/tic-tac-toe.full.js', ['browserify']);
...
gulp.task('browser-serve', function() {
    ...
    gulp.watch('./dist/tic-tac-toe.bundle.js').on('change', browserSync.reload);
});

gulp.task('browser-proxy', function() {
    browserSync.init({
        proxy: {
            target: "http://localhost:8888/tic-tac-toe/example/index.html",
            ws: true
        }
    });
